...
| FR-05 | REFACTOR | The target patient information architecture must contain Home, Chat, Care, Community, More, and Profile while preserving access to every current top-level feature. Phase 2.2B must validate whether mobile uses all six persistently or five activity destinations plus persistent Profile access in the header. | Route inventory has no orphaned destination, and 360px, 375px, 390px, and 430px checks confirm accessible labels and target sizes. |
...
| FR-22 | PRESERVE | Chat and community must remain first-class destinations. Education, games, advocacy, genotype education, and research must remain reachable through More or another explicitly approved secondary path. | Route mapping and smoke checks cover each surface. |
